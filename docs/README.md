...
- [ ] 각각의 할인 방법은, DiscountStrategy라는 추상화 인터페이스를 구현하였습니다.
    ...
    - DiscountProvider 클래스에 각 구현 클래스 객체 리스트를 상수로 정의하였고, 이 리스트를 이용하여 전체 할인과 관련된 정보를 얻어냅니다.


- [ ] 마지막 미션인 만큼, 가능한 많은 경우를 테스트하려고 했습니다. 그러다 보니 테스트 케이스가 203개로 많이 많습니다..
...
- [x] DiscountProvider 클래스의 상수에는 모든 할인 정보가 담긴 리스트가 정의되어 있습니다.
    - 할인 정보가 추가되거나 삭제되면, 상수에 정의된 값만 수정하면 되도록 설정하였습니다.
    - 상수에 정의된 할인 리스트와, 샴페인 증정을 포함한 총 혜택 금액, 실제로 할인되는 금액, 할인 내역을 반환합니다.
