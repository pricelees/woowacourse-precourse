...
- [ ] 날짜와 관련된 값을, 정수값이 아닌 LocalDateTime으로 사용하여 확장성을 고려하였습니다.
...
- [ ] 가장 기본이 되는 컨트롤러를 MainController, 플래너의 기능을 담당하는 컨트롤러는 PlannerControllerable로 추상화 한 뒤 구현하였습니다.


- [ ] 컨트롤러는 도메인에 직접 접근하지 않고, View에서 얻어낸 입력값을 바탕으로 CustomerService 객체를 만들고 이용합니다.
    - CustomerService는 구매 금액이 1만원 이상인 경우(이벤트 참여 가능)와 아닌 경우로 구성되어 있습니다.


- [ ] 각각의 할인 방법은, DiscountStrategy라는 추상화 인터페이스를 구현하였습니다.
    - InputView에서는 공통된 예외 메시지를 사용하나, 각 DiscountStrategy의 구현 클래스에서의 예외 검증은 각 검증에 맞는 예외 메시지를 출력합니다.
    - 전체 할인 금액을 얻어낼 때엔, List<DiscountStrategy>와 스트림을 이용하여 처리합니다.


- [ ] 마지막 미션인 만큼, 가능한 많은 경우를 테스트하려고 했습니다. 그러다 보니 테스트 케이스가 219개로 많이 많습니다..
...
- [x] DiscountErrorMessage의 상수에는 각 할인과 관련된 예외 메시지가 저장되어 있습니다.
...
- [x] constants 패키지 안의 view 패키지엔 View에서 사용되는 상수들을 저장하였습니다.
...
- [x] CustomerServiceWithDiscount 클래스는, 이벤트 참여가 가능할 때(1만원 이상 구매) 이용되며, BenefitsContainer와 BadgeContainer를
  이용하여 정보를 얻어냅니다.
...
- [x] CustomerServiceFinder는 입력된 LocalDateTime과 Map<Menu, Integer>를 이용하여 Customer 객체를 만들 고, 이벤트 참여 여부에 따라
...
- [x] DiscountStrategy는 각 할인 정보의 추상화 인터페이스이며, 연도와 월을 검증하는 메서드는 default로 제공합니다.
    - 할인의 이름, 할인되는 금액, 전체적인 예외 검증은 각 구현 클래스에서 재정의 하도록 설정하였습니다.
    - 할인 금액을 얻어낼 때엔, Customer 객체를 이용하여 얻어냅니다.


- [x] ChristmasDiscount는 크리스마스 디데이 할인 정보를 제공하며, 23년 12월 1일부터 25일까지인지에 대한 예외 검증을 추가로 진행합니다.


- [x] SpecialDiscount는 특별 할인 정보를 제공하며, 달력 상의 특별 할인 일에 해당되는지에 대한 예외 검증을 추가로 진행합니다.


- [x] WeekdayDiscount는 평일 할인 정보를 제공하며, 평일인지에 대한 예외 검증을 추가로 진행합니다.


- [x] WeekendDiscount는 주말 할인 정보를 제공하며, 주말인지에 대한 예외 검증을 추가로 진행합니다.


- [x] ChampagneDiscount는 샴페인 증정 여부를 제공하며, 할인 전 금액이 12만원 이상인지에 대한 예외 검증을 추가로 진행합니다.

---

### 기능 구현 사항 - domain 패키지

- [x] BadgeContainer 클래스는 필드값으로 할인 후 금액을 가지며, 이 금액에 따라 제공되는 배지의 이름을 반환합니다.
    - 할인 후 금액이 음수인지에 대한 예외 검증을 진행합니다. 할인 후 금액이 할인 전 금액보다 클 수 없기 때문입니다.


- [x] BenefitsContainer 클래스는 Customer 클래스 객체를 필드로 가지며, 이 필드를 통해 List<DiscountStrategy>를 얻어냅니다.
    - 얻어낸 리스트와 스트림을 통해, 샴페인 증정을 포함한 총 혜택 금액, 실제로 할인되는 금액, 할인 후의 금액, 할인 내역을 반환합니다.
...
- [x] Customer 클래스는, LocalDateTime과 SelectedMenu를 이용하여 고객과 관련된 기능을 제공합니다.
    - 날짜가 2023년 12월인지에 대한 예외 검증을 진행합니다.

---

### 기능 구현 사항 - util 패키지

- [x] DiscountProviderByDate 클래스는, LocalDateTime 객체를 받아 2023년 12월인지에 대한 예외 검증을 한 뒤, "일" 정보를 이용하여
  할인 정보 리스트인 List<DiscountStrategy>를 반환합니다.
...
- [x] Customer 클래스에선, LocalDateTime과 SelectedMenu를 필드로 가지며, SelectedMenu는 자체 검증이 있으므로 날짜에 대해 검증합니다.
    - "일"에 대한 검증은 LocalDateTime이 진행하므로, 2023년인지, 12월인지에 대한 검증을 각각 진행합니다.


- [x] DiscountStrategy에선 연도와 월을 검증하는 메서드는 default로 지정하고, 구현 클래스에서 이 메서드를 이용하고 추가적인 검증을 진행합니다.
    - ChristmasDiscount는 일자가 1일부터 25일까지인지를 추가로 확인합니다.
    - ChampagneDiscount는 주문 금액이 12만원 이상인지를 추가로 확인합니다.
    - SpecialDiscount는 방문 일자가 특별 할인일에 해당되는지를 추가로 확인합니다.
    - WeekDayDiscount, WeekendDiscount는 방문 일자가 평일, 주말인지를 추가로 확인합니다.
...
- [x] DiscountStrategy 인터페이스에 대한 테스트
    - 모든 구현 클래스를 넣고, default 메서드인 연,월 검증에 대해 테스트합니다.


- [x] DiscountStrategy의 각 구현 클래스에 대한 테스트
    - 예외 발생에 대해 테스트하고, 각 할인 조건에 맞는 할인 금액을 정확히 반환하는지 확인합니다.


- [x] BadgeContainer 클래스에 대한 테스트
    - 생성자에 음수인 값을 넣고 예외를 발생시키는지 확인합니다.
    - -경계값을 기반으로, 알맞는 배지 이름을 반환하는지 확인합니다.


- [x] BenefitsContainer 클래스에 대한 테스트
    - 모든 메뉴를 선택하고, 요일을 다르게 지정하여 정확한 할인 금액, 혜택 금액, 할인 후 예상 금액을 반환하는지 확인합니다.


- [x] Customer 클래스에 대한 테스트
    - 잘못된 날짜를 넣었을 때, 예외를 발생시키는지 확인합니다.
...
- [x] CustomerServiceWithDiscount 클래스에 대한 테스트
    ...
    - 증정 메뉴와, 배지는 1일부터 31일까지 모두 진행되므로, 날짜를 랜덤으로 지정한 뒤 정확한 내역을 반환하는지 확인합니다.
    - 총 혜택 금액, 할인 내역, 예상 결제 금액은 날짜를 다르게 입력하며 각각 다른 경우에 대해 확인합니다.


- [x] CustomerServiceWithoutDiscount 클래스에 대한 테스트
    - 1만원 미만이 되도록 메뉴를 지정하고, 날짜는 무관하기에 랜덤값으로 지정합니다.
    - 이벤트와 관련된 출력 내용이 "없음" 또는 "0원"으로 출력되는지 확인합니다.


- [x] DiscountProviderByDate 클래스에 대한 테스트
    - 2023년 12월이 아닐 때의 예외 발생을 확인합니다.
    - 날짜를 다르게 바꿔가며, 할인 종류를 정확하게 반환하는지 확인합니다.
